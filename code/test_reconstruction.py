...
from utilities import _call_kwargs_deploy_train_run, _call_kwargs_deploy_test_run
import pickle
...
def run_one(variable_name, variable_range, output_dir):
    ...
    training_run_names = ['G-NMF','C-NMF','G-CoNE','C-CoNE','HNMF','CoNE','SCoNE','SCoNE(Fro)','RGWAS','MVBC']
    ...
    plan["out_path"] = f"{output_dir}/"+ plan["split"] + "_" + plan["job_id"].astype(str)
    plan.to_csv(f"{output_dir}/run_plan.csv", index=False)
    ...
    tune_record.to_csv(f"{output_dir}/run_record_train.csv", index=False)
    ...
    train_record.to_csv(f"{output_dir}/run_record_tune.csv", index=False)
...
if __name__ == "__main__": 
    ...
    summary_sim, summary_relerror_G, summary_relerror_C, all_testing_results, tuning_results = run_one("rho",  [1], output_dir=f'{root_dir}/output/models')
