...
def compute_loss(X,X_hat,loss_type,xp=np):
    if loss_type == 'kl_div':
        ...
        #return xp.sum(-xp.multiply(X,xp.log(X_hat)) + X_hat + gammaln(X + 1)) # log(X!) can help stabilize/make pos.
    elif loss_type == 'fro':
        return (1/2)*xp.dot((X - X_hat).ravel(), (X - X_hat).ravel())
...
def l1_norm(x,xp=np):
    if x is not None:
        return xp.sum(xp.abs(x),dtype=xp.float64)
    else:
        return 0

def l2_norm_squared(x,xp=np):
    return xp.sum(x * x, dtype=xp.float64)

def compute_jac(sample_matrix, X, X_hat, loss_type, for_W=False, xp=np):
    '''
...
def get_X_hat(W,H,Z,U,loss_type,xp=np):
    if Z is not None:
        X_hat = W@H.T + Z@U.T
    else:
        X_hat = W@H.T
    if loss_type in ['kl_div','fro']:
        X_hat = xp.clip(X_hat, 1e-9, xp.inf) # clipping for log purposes
    elif loss_type is None:
        X_hat = None
    else: assert True == False, f"{loss_type} not a valid loss type, should be one of 'kl_div', 'fro'"
    return X_hat

def total_loss(G, C, Z, W, H_G, H_C, U_G, U_C, alpha, lambda_H_G, lambda_H_C, lambda_Gloss, G_loss_type, C_loss_type, xp=np):
    # define nec. elements
    if G_loss_type is not None:
        G_hat = get_X_hat(W,H_G,Z,U_G,G_loss_type,xp=xp)
        G_loss = compute_loss(G,G_hat,loss_type=G_loss_type,xp=xp)
    else: G_loss = 0
    if C_loss_type is not None:
        C_hat = get_X_hat(W,H_C,Z,U_C,C_loss_type,xp=xp)
        C_loss = compute_loss(C,C_hat,loss_type=C_loss_type,xp=xp)
    else: C_loss = 0

    l2_regularization = alpha*l2_norm_squared(W,xp=xp)
    l1_regularization = lambda_H_G*l1_norm(H_G,xp=xp) +  lambda_H_C*l1_norm(H_C,xp=xp) # l1 reg.
...
def make_fg_W(shape, G, C, Z, W, H_G, H_C, U_G, U_C, lambda_Gloss, alpha, G_loss_type, C_loss_type, xp=np):
    ...
    def f(x):
        ...
        if G_loss_type is not None:
            G_hat = _forwardG(x)
            G_loss = compute_loss(G,G_hat,G_loss_type,xp=xp)
        else:
            G_loss = 0

        if C_loss_type is not None:
            C_hat = _forwardC(x)
            C_loss =  compute_loss(C,C_hat,C_loss_type,xp=xp)
        else:
            C_loss = 0
        # compute fun
        loss = lambda_Gloss*G_loss + C_loss + (alpha / 2)*l2_norm_squared(W,xp=xp) # last term adds L2 norm on W
        return loss
    ...
    def g(x):
        ...
        if G_loss_type is not None:
            G_hat = _forwardG(x)
            # compute jac
            GW_wrt_G = compute_jac(H_G, G, G_hat, G_loss_type, for_W=True, xp=xp)
        else:
            GW_wrt_G = xp.zeros((W.shape[0],W.shape[1]))

        if C_loss_type is not None:
            C_hat = _forwardC(x)
            # compute jac
            GW_wrt_C = compute_jac(H_C, C, C_hat, C_loss_type, for_W=True, xp=xp)
        else:
            GW_wrt_C = xp.zeros((W.shape[0],W.shape[1]))
...
def make_fg_HG(shape, G, C, Z, W, H_G, H_C, U_G, U_C, lambda_Gloss, G_loss_type, C_loss_type, xp=np):
    ...
    def _forward(x):
        ...
        G_hat = get_X_hat(W,H_G,Z,U_G,G_loss_type,xp=xp) # could cut down on matrix multiplications if use precalculated Z@U_G.T for fun and jac
        return G_hat
    def f(x):
        G_hat = _forward(x)
        G_loss = compute_loss(G,G_hat,G_loss_type,xp=xp)
    ...
    def g(x):
        G_hat = _forward(x)
        # compute jac
        GH_G = lambda_Gloss*compute_jac(W, G, G_hat, G_loss_type, xp=xp)
        return GH_G.flatten(order='F')

    return f,g

def make_fg_HC(shape, G, C, Z, W, H_G, H_C, U_G, U_C, lambda_Gloss, G_loss_type, C_loss_type, xp=np):
    ...
    def g(x):
        C_hat = _forward(x)
        # compute jac
        GH_C = compute_jac(W, C, C_hat, C_loss_type, xp=xp)
        return GH_C.flatten(order='F')

    return f,g

def make_fg_UG(shape, G, C, Z, W, H_G, H_C, U_G, U_C, lambda_Gloss, G_loss_type, C_loss_type, xp=np):
    ...
    def g(x):
        G_hat = _forward(x)
        # compute jac
        GU_G = lambda_Gloss*compute_jac(Z, G, G_hat, G_loss_type, xp=xp)
        return GU_G.flatten(order='F')

    return f,g

def make_fg_UC(shape, G, C, Z, W, H_G, H_C, U_G, U_C, lambda_Gloss, G_loss_type, C_loss_type, xp=np):
    ...
    def g(x):
        C_hat = _forward(x)
        # compute jac
        GU_C = compute_jac(Z, C, C_hat, C_loss_type, xp=xp)
        return GU_C.flatten(order='F')

    return f,g

def soft_thresh(z, tau, xp=np):
    # elementwise prox for L1: sign(z)*max(|z|-tau,0)
    return xp.sign(z) * xp.maximum(xp.abs(z) - tau, 0.0)

def proj_nonneg(x, xp=np):
    # If already feasible, return x as-is (avoids an allocation most iterations)
    if x.min() >= 0:
        return x
    return xp.maximum(x, 0)

def armijo_suff_decrease_cond(f_new,f,g,x_new,x,sigma, xp=np):
    return f_new - f <= sigma*xp.dot(g.ravel(),(x_new-x).ravel())

def pgd_armijo(fun, grad, x0, max_iter=500, rho=0.1, sigma=1e-4, ftol=1e-12, l1=0,max_ls=10, xp=np):
    """
    ...
    """
    x = proj_nonneg(x0, xp=xp)
    n=1

    def x_and_s(step, x, g):
        z = x - step * g
        if l1 > 0:
            z = soft_thresh(z, step * l1, xp=xp)
        x_new = proj_nonneg(z, xp=xp)
        s = x_new - x
        return x_new, s
    ...
    for it in range(max_iter):
        ...
        if f_new - f <= sigma * xp.vdot(g, s):
            ...
            for _ in range(max_ls):
                ...
                if xp.allclose(x_next, x_new):
                    break
                f_next = fun(x_next)
                if not (f_next - f <= sigma * xp.vdot(g, s_next)):
                    break
            ...
            for _ in range(max_ls):
                ...
                x_next, s_next = x_and_s(n_next, x, g)
                f_next = fun(x_next)
                if f_next - f <= sigma * xp.vdot(g, s_next):
                    n, x_new, s, f_new = n_next, x_next, s_next, f_next
                    break
                n = n_next
            else:
...
def alternating_opt(
    ...
    def one_block_update(name, X):
        ...
        if name == "W":
            f,g = make_fg_W(W.shape, G, C, Z, W, H_G, H_C, U_G, U_C, lambda_Gloss, alpha, G_loss_type, C_loss_type, xp=xp)
            l1 = 0
        elif name == "H_G":
            f,g = make_fg_HG(H_G.shape, G, C, Z, W, H_G, H_C, U_G, U_C, lambda_Gloss, G_loss_type, C_loss_type, xp=xp)
            l1 = lambda_H_G
        elif name == "H_C":
            f,g = make_fg_HC(H_C.shape, G, C, Z, W, H_G, H_C, U_G, U_C, lambda_Gloss, G_loss_type, C_loss_type, xp=xp)
            l1 = lambda_H_C
        elif name == "U_G":
            f,g = make_fg_UG(U_G.shape, G, C, Z, W, H_G, H_C, U_G, U_C, lambda_Gloss, G_loss_type, C_loss_type, xp=xp)
            l1 = 0
        elif name == "U_C":
            f,g = make_fg_UC(U_C.shape, G, C, Z, W, H_G, H_C, U_G, U_C, lambda_Gloss, G_loss_type, C_loss_type, xp=xp)
            l1 = 0
        else:
            raise ValueError(f"Unknown block {name}")
        x = pgd_armijo(f, g, x0, max_iter=max_inner, rho=rho, sigma=sigma, ftol=inner_ftol, l1=l1, max_ls=max_ls, xp=xp)
    ...
    if use_gpu:
        try:
            ...
            raise ImportError("CuPy is required when use_gpu=True.")
        xp = cp
    else:
        xp = np
    ...
    if Z is not None: Z = xp.asarray(Z)
    
    W = xp.asarray(W)
    if H_G is not None: H_G = xp.asarray(H_G)
    ...
    f_prev, G_loss, C_loss, l2_regularization, l1_regularization = total_loss(G, C, Z, W, H_G, H_C, U_G, U_C, alpha, lambda_H_G, lambda_H_C, lambda_Gloss, G_loss_type, C_loss_type, xp=xp)
    for _ in range(max_outer):
        ...
        f_cur, G_loss, C_loss, l2_regularization, l1_regularization = total_loss(G, C, Z, W, H_G, H_C, U_G, U_C, alpha, lambda_H_G, lambda_H_C, lambda_Gloss, G_loss_type, C_loss_type, xp=xp)
        ...
        # record matrix norms
        loss_dict['W_norm'].append(xp.linalg.norm(W))
        if G_loss_type is not None:
            loss_dict['H_G_norm'].append(xp.linalg.norm(H_G))
            if Z is not None:
                loss_dict['U_G_norm'].append(xp.linalg.norm(U_G))
        if C_loss_type is not None:
            loss_dict['H_C_norm'].append(xp.linalg.norm(H_C))
            if Z is not None:
                loss_dict['U_C_norm'].append(xp.linalg.norm(U_C))

        # record sparsity
        loss_dict['W_sparsity'].append(100*xp.count_nonzero(W == 0)/ W.size)
        if G_loss_type is not None:
            loss_dict['H_G_sparsity'].append(100*xp.count_nonzero(H_G == 0)/ H_G.size)
            if Z is not None:
                loss_dict['U_G_sparsity'].append(100*xp.count_nonzero(U_G == 0)/ U_G.size)
        if C_loss_type is not None:
            loss_dict['H_C_sparsity'].append(100*xp.count_nonzero(H_C == 0)/ H_C.size)
            if Z is not None:
                loss_dict['U_C_sparsity'].append(100*xp.count_nonzero(U_C == 0)/ U_C.size)
    ...
    if not test:
        if post_hoc_rescale is True:
            ...
            norms = xp.linalg.norm(W, axis=0)
            norms[norms == 0] = 1.0
        else:
            norms = xp.ones(W.shape[1], dtype=W.dtype)
        W_normed = W / norms
        factor_matrices = {"W":W_normed}
        # scale rows of H_G and H_C
        if G_loss_type is not None:
            H_G_normed = H_G * norms[xp.newaxis, :]
            factor_matrices["H_G"] = H_G_normed
            if Z is not None:
                factor_matrices["U_G"] = U_G
        if C_loss_type is not None:
            H_C_normed = H_C * norms[xp.newaxis, :]
    ...
    if use_gpu:
        factor_matrices = {
            ...
            for key, value in factor_matrices.items()
        }
